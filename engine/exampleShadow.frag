...
uniform highp sampler3D jitter;  // jitter map
uniform sampler2D spot;    // projected spotlight image
uniform highp sampler2DShadow shadowMap;  // shadow map
...
void main(void)  {    
    ...
    for (int i = 0; i<SAMPLES_COUNT_DIV_2 - 4; i++) {      
        ...
        smCoord.xy = offset.zw * fsize + shadowMapPos;      
        shadow += texture2DProj(shadowMap, smCoord)* INV_SAMPLES_COUNT;    }  }    // all done Ð modulate lighting with the computed shadow value  
        vec3 color = texture2D(decal, texCoord).xyz;    
        gl_FragColor.xyz = (color * NdotL + pow(max(dot(R, L), 0), 64)) * 
        shadow * texture2DProj(spot, shadowMapPos) + color * 0.1;  
    }